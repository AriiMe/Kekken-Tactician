...
function Punishment({ title, rows }) {
  return <section className="t7-section"><h2>{title}</h2><dl className="t7-punishers">
    {rows.map((row, i) => <div key={`${row.frames}-${i}`}><dt><strong>i{row.frames}</strong><small>vs −{row.frames} or worse</small></dt>
      <dd>{renderInputImage(row.input)}</dd></div>)}
  </dl></section>;
}
        ...
        : <><nav className="t7-sections" aria-label="Guide sections"><a href="#t7-combos">Combos</a><a href="#t7-punish">Punishers</a><a href="#t7-throws">Throws</a><a href="#t7-wall">Wall combos</a></nav>
          <div className="t7-guide" key={character.slug}>
            <section className="t7-section" id="t7-combos"><h2>Main Combos</h2>
              <div className="t7-combos">{character.combos.map((combo, index) => <article className="t7-combo" key={index}>
                <h3>Launcher{combo.launchers.length > 1 ? 's' : ''}</h3><div className="t7-launchers">{combo.launchers.map(input => <div key={input}>{renderInputImage(input)}</div>)}</div>
                <h3>Follow-up</h3><div className="t7-input">{renderInputImage(combo.followUp)}</div>{combo.notes && <p>{combo.notes}</p>}
              </article>)}</div>
            </section>
            <div id="t7-punish"><div className="t7-punishment"><Punishment title="Standing Punishers" rows={character.punishers.standing} /><Punishment title="While Rising Punishers" rows={character.punishers.crouching} /></div>
